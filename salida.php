<?php
	...
	if(isset($_POST['enviar'])){
		$db = mysqli_connect('localhost','alejan14_style','style16','alejan14_style');
		
		if(!$db) echo "error ".mysqli_error($db);
		
		$permitidos=array(4,5,6,7,8,9,10,11,12);
		$existe_usuario="select * from usuarios where usuario='".$_POST['usuario']."' and password='".$_POST['contra']."'";
				 ...
				 if($row_cnt<1){
					 ...
					 while($dato=mysqli_fetch_assoc($consulta_existe)){
						 ...
						 if(in_array($dato['id_tipo_usuario'],$permitidos)){
						 	...
						 	if($row_cnt_verifica<1){
								 echo("NO registró asistencia el día de hoy");
							
							}else{
								echo $query="UPDATE asistencia_empleado SET `hora_salida`='".date("G:i")."' where `id_usuario`='".$dato['id_usuario']."' and `fecha`='".date("Y-m-d")."'";
